(() => {
  ...
  function signature() { const s = read(KEY,{activeId:null,trips:[]}), a = read(ARCHIVE_KEY,[]); return `${s.activeId}|${s.trips.map(t=>`${t.id}:${t.name}`).join('|')}|${a.map(x=>x.plan?.id).join('|')}`; }
  function render() {
    sync(); document.querySelector('.vela-trip-index')?.remove();
    ...
    s.trips.slice(0,12).forEach((trip,i)=>{ const b=document.createElement('button'); b.className='vela-trip-tab'+(trip.id===s.activeId?' active':''); b.title=trip.name||'Untitled trip'; b.innerHTML=`<span>${String(i+1).padStart(2,'0')}</span><b>${(trip.name||'Untitled').slice(0,18)}</b>`; b.onclick=()=>switchTo(trip.id); rail.append(b); });
  ...
  function showArchived(items) {
    document.querySelector('.vela-trip-archive-sheet')?.remove(); const back=document.createElement('div'); back.className='vela-trip-archive-sheet'; const sheet=document.createElement('section'); sheet.className='vela-trip-archive-inner';
    sheet.innerHTML='<div class="vela-trip-archive-head"><div><span>ARCHIVE</span><h2>Archived Trips</h2></div><button>Close</button></div>'; const list=document.createElement('div'); list.className='vela-trip-archive-list';
  ...
  const start=()=>{ let last=''; const tick=()=>{ const s=signature(); if(s!==last){last=s;render();} }; tick(); setInterval(tick,1200); };
