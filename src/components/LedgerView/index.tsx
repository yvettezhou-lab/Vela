...
export const LedgerView:React.FC=()=>{
 const currentTrip=useVelaStore(s=>s.getCurrentTrip()); const ledger=useVelaStore(s=>s.getCurrentTrip()?.ledger); const deleteLedgerEntry=useVelaStore(s=>s.deleteLedgerEntry); const updateLedgerEntry=useVelaStore(s=>s.updateLedgerEntry); const [filter,setFilter]=useState<'all'|'pending'>('all');
 ...
 const fillCny=(entry:LedgerEntry)=>{
   const raw=window.prompt('填写实际人民币金额（以银行/信用卡账单为准）', ''); if(raw===null)return; const value=Number(raw); if(!Number.isFinite(value)||value<=0){window.alert('请输入大于 0 的人民币金额。');return;}
   ...
   updateLedgerEntry(currentTrip.id,entry.id,{...entry,cnyEquivalent:value,isPending:false,allocations,updatedAt:Date.now()});
 };
 return <section className="max-w-2xl mx-auto p-6 bg-white shadow rounded-lg mt-8">
  ...
  {filtered.length===0?<div className="p-5 border rounded-lg bg-gray-50 text-sm text-gray-600">{filter==='pending'?'No entries waiting for CNY.':'No ledger entries yet.'}</div>:<div className="space-y-3">{filtered.map(entry=><article key={entry.id} className="flex items-start justify-between gap-4 p-4 border rounded-lg">
   <div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><span className="font-semibold truncate">{ENTRY_LABELS[entry.entryType]}</span>{entry.isPending&&<span className="px-2 py-0.5 rounded-full bg-amber-100 text-amber-800 text-xs">CNY 待补</span>}{!entry.includeInCost&&<span className="px-2 py-0.5 rounded-full bg-stone-100 text-stone-700 text-xs">不计入旅行成本</span>}</div>
   ...
   <div className="flex shrink-0 items-center gap-1">{entry.isPending&&<button type="button" onClick={()=>fillCny(entry)} className="rounded-md border px-2 py-1.5 text-xs text-slate-700 hover:bg-gray-50">补 CNY</button>}<button type="button" aria-label={'Delete '+ENTRY_LABELS[entry.entryType]+' entry'} title="Delete entry" className="p-2 rounded-md border text-red-700 hover:bg-red-50" onClick={()=>deleteLedgerEntry(currentTrip.id,entry.id)}><Trash2 size={17}/></button></div>
  </article>)}</div>}
 </section>;
};
