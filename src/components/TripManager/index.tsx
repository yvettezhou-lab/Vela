...
export const TripManager: React.FC = () => {
  ...
  const startEditingDates = (tripId: string) => {
    ...
    setDraftStart(toDateInput(trip.startDate));
    setDraftEnd(toDateInput(trip.endDate));
  };
  ...
  const saveDates = () => {
    ...
    setError(null);
    try {
      updateTripDates(editingTripId, fromDateInput(draftStart), fromDateInput(draftEnd));
      setEditingTripId(null);
    } catch (caught) {
      setError(lifecycleError(caught, 'dates'));
    }
  };

  const startTrip = (tripId: string) => {
    setError(null);
    try { updateTripStatus(tripId, 'traveling'); } catch (caught) { setError(lifecycleError(caught, 'start')); }
  };
  ...
  return (
    <section style={styles.container} aria-label="Trip Manager">
      <div style={styles.header}>
        <div><div style={styles.eyebrow}>TRIP MANAGER</div><h2 style={styles.title}>Your Journeys</h2></div>
                    ...
                    <div style={styles.cardMain}>
                      ...
                      {isEditing && (
                        <div style={styles.dateEditor}>
                          <label style={styles.dateField}><span>Start</span><input type="date" value={draftStart} onChange={(event) => setDraftStart(event.target.value)} /></label>
                          <label style={styles.dateField}><span>End</span><input type="date" value={draftEnd} onChange={(event) => setDraftEnd(event.target.value)} /></label>
                          <div style={styles.editorActions}><button type="button" onClick={() => setEditingTripId(null)} style={styles.cancelButton}>Cancel</button><button type="button" onClick={saveDates} style={styles.confirmButton}>Save dates</button></div>
                        </div>
                      )}
                    </div>
                    <div style={styles.actions}>
                      <button type="button" onClick={() => startEditingDates(trip.id)} style={styles.dateButton}>{isEditing ? 'Editing' : 'Edit dates'}</button>
                      {status === 'planning' && <button type="button" onClick={() => startTrip(trip.id)} style={styles.primaryButton}>Start Trip</button>}
                      {status === 'traveling' && <button type="button" onClick={() => requestEndJourney(trip.id)} style={styles.secondaryButton}>End Journey</button>}
                    </div>
                  </article>
                );
              })}
      ...
      {endingTrip && (
        <div role="presentation" style={styles.backdrop} onMouseDown={(event) => { if (event.target === event.currentTarget) setEndingTripId(null); }}>
          <div role="dialog" aria-modal="true" aria-labelledby="end-journey-title" style={styles.dialog}>
            <div style={styles.dialogEyebrow}>END JOURNEY</div><h3 id="end-journey-title" style={styles.dialogTitle}>Finish {endingTrip.title}?</h3>
            <p style={styles.dialogBody}>This will close the active journey and move it to Achieve. Its existing ledger entries remain unchanged and will be reflected in completed journeys.</p>
            <div style={styles.dialogActions}><button type="button" onClick={() => setEndingTripId(null)} style={styles.cancelButton}>Keep traveling</button><button type="button" onClick={confirmEndJourney} style={styles.confirmButton}>End Journey</button></div>
          </div>
        </div>
      )}
    </section>
...
const styles: Record<string, React.CSSProperties> = {
  ...
  eyebrow: { fontSize: 10, letterSpacing: '0.16em', opacity: 0.55 }, title: { margin: '3px 0 0', fontSize: 24 }, count: { fontSize: 12, opacity: 0.6 },
  error: { display: 'flex', justifyContent: 'space-between', gap: 12, alignItems: 'center', marginBottom: 14, padding: '10px 12px', border: '1px solid #d7a5a5', borderRadius: 8, background: '#fff5f5', color: '#8a2020', fontSize: 13 },
  dismiss: { border: 0, background: 'transparent', fontSize: 18, cursor: 'pointer', color: 'inherit' }, group: { marginTop: 18 }, groupHeader: { display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }, groupTitle: { margin: 0, fontSize: 14, letterSpacing: '0.04em' }, groupCount: { minWidth: 20, padding: '2px 6px', borderRadius: 999, background: '#f0eee8', fontSize: 11, textAlign: 'center' }, empty: { padding: '12px 0', color: '#7b7d82', fontSize: 13 }, list: { display: 'grid', gap: 8 },
  ...
  dateEditor: { display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginTop: 10, padding: 10, borderRadius: 8, background: '#f7f5ef' }, dateField: { display: 'grid', gap: 4, fontSize: 10, letterSpacing: '0.04em', color: '#62656b' }, dateInput: {}, editorActions: { gridColumn: '1 / -1', display: 'flex', justifyContent: 'flex-end', gap: 7 },
