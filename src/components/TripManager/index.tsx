...
const STATUS_LABELS: Record<TripStatus, string> = { traveling: 'Traveling', planning: 'Planning', achieve: 'Achieve', cancelled: 'Cancelled' };
const STATUS_ORDER: TripStatus[] = ['traveling', 'planning', 'achieve', 'cancelled'];
...
function lifecycleError(error: unknown, action: 'start' | 'archive' | 'cancel'): string {
  ...
  if (action === 'cancel') return `Cannot cancel journey: ${message}`;
...
export const TripManager: React.FC = () => {
  ...
  const confirmCancelJourney = () => {
    if (!cancellingTripId) return; setError(null);
    try { updateTripStatus(cancellingTripId, 'cancelled'); setCancellingTripId(null); }
    catch (caught) { setError(lifecycleError(caught, 'cancel')); }
  };
  ...
  const cancellingTrip = cancellingTripId ? trips.find((trip) => trip.id === cancellingTripId) : null;
  ...
  return (
    <section style={styles.container} aria-label="Trip Manager">
      ...
      {cancellingTrip && <div role="presentation" style={styles.backdrop} onMouseDown={(event) => { if (event.target === event.currentTarget) setCancellingTripId(null); }}><div role="dialog" aria-modal="true" aria-labelledby="cancel-journey-title" style={styles.dialog}><div style={styles.dialogEyebrow}>CANCEL JOURNEY</div><h3 id="cancel-journey-title" style={styles.dialogTitle}>Cancel {cancellingTrip.title}?</h3><p style={styles.dialogBody}>This keeps the journey and any existing records, but removes it from active travel. You can still find it under Cancelled.</p><div style={styles.dialogActions}><button type="button" onClick={() => setCancellingTripId(null)} style={styles.cancelButton}>Keep Journey</button><button type="button" onClick={confirmCancelJourney} style={styles.confirmButton}>Cancel Journey</button></div></div></div>}{endingTrip && <div role="presentation" style={styles.backdrop} onMouseDown={(event) => { if (event.target === event.currentTarget) setEndingTripId(null); }}><div role="dialog" aria-modal="true" aria-labelledby="end-journey-title" style={styles.dialog}><div style={styles.dialogEyebrow}>END JOURNEY</div><h3 id="end-journey-title" style={styles.dialogTitle}>Finish {endingTrip.title}?</h3><p style={styles.dialogBody}>This will close the active journey and move it to Achieve. Its existing ledger entries remain unchanged and will be reflected in completed journeys.</p><div style={styles.dialogActions}><button type="button" onClick={() => setEndingTripId(null)} style={styles.cancelButton}>Keep traveling</button><button type="button" onClick={confirmEndJourney} style={styles.confirmButton}>End Journey</button></div></div></div>}
    </section>
  );
};
...
const styles: Record<string, React.CSSProperties> = {
  ...
  actions: { flexShrink: 0, display: 'flex', flexDirection: 'column', gap: 6 }, dateButton: { border: '1px solid #d7d4ca', borderRadius: 7, padding: '7px 10px', background: '#fffdf8', color: '#62656b', cursor: 'pointer', fontSize: 11 }, primaryButton: { border: 0, borderRadius: 7, padding: '8px 11px', background: '#172033', color: '#fff', cursor: 'pointer', fontSize: 12 }, cancelJourneyButton: { border: 0, background: 'transparent', color: '#8a5b54', cursor: 'pointer', fontSize: 10, padding: '5px 8px' }, secondaryButton: { border: '1px solid #c9c5ba', borderRadius: 7, padding: '8px 11px', background: '#fff', color: '#172033', cursor: 'pointer', fontSize: 12 },
