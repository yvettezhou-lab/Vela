...
export const LogbookView = ({ annualReflection, activeTrip, plannedTrips, tripInsights, reflectionTrips }: LogbookViewProps) => {
  ...
  return (
    <main className="logbook-page">
      ...
      {activeInsight && activeTrip && <section className="logbook-section logbook-live-section"><div className="logbook-section-heading"><div><span className="logbook-overline">IN MOTION NOW</span><h2>{activeTrip.title || activeTrip.destination || 'Current journey'}</h2></div><span className="logbook-live-badge">LIVE</span></div><button type="button" className="logbook-trip-card logbook-trip-button" onClick={() => openTrip(activeTrip)}><div className="logbook-trip-top"><span>{activeTrip.destination || 'Current journey'}</span><strong>{activeInsight.totalDays} DAYS</strong></div><div className="logbook-trip-body"><div><small>SPEND SO FAR</small>{Object.entries(activeInsight.totalExpenditure).map(([currency, amount]) => <strong key={currency}>{number(amount)} <em>{currency}</em></strong>)}</div><div><small>PER DAY</small>{Object.entries(activeInsight.averageCostPerDay).map(([currency, amount]) => <strong key={currency}>{number(amount)} <em>{currency}</em></strong>)}</div><div><small>ENTRIES</small><strong>{activeTrip.ledger.length}</strong></div></div><span className="logbook-drill-hint">View journey →</span></button></section>}
      ...
      {(annualDrilldown || selectedTrip) && <div className="logbook-drill-overlay" role="dialog" aria-modal="true" aria-label={selectedTrip ? 'Journey details' : 'Annual journey list'}>
        <div className="logbook-drill-sheet">
          ...
          {!selectedTrip && <>
            ...
            <div className="logbook-drill-list">{reflectionTrips.map((trip) => { const insight = tripInsights.find((item) => item.tripId === trip.id); return <button type="button" className="logbook-drill-trip" key={trip.id} onClick={() => openTrip(trip)}><div><strong>{trip.title || trip.destination || 'Untitled journey'}</strong><span>{trip.destination || 'Destination not set'}</span></div><div><strong>{insight?.totalDays ?? '—'} d</strong><span>{trip.status === 'traveling' ? 'LIVE' : 'ACHIEVED'}</span></div><span className="logbook-chevron">›</span></button>; })}</div>
          </>}

          {selectedTrip && selectedInsight && <>
            <div className="logbook-drill-trip-heading"><div><span className="logbook-overline">{selectedTrip.status.toUpperCase()}</span><h2 className="logbook-drill-title">{selectedTrip.title || selectedTrip.destination || 'Untitled journey'}</h2><p>{selectedTrip.destination || 'Destination not set'} · {date(selectedTrip.startDate)} — {date(selectedTrip.endDate)}</p></div><span className="logbook-drill-days">{selectedInsight.totalDays}<small>DAYS</small></span></div>
