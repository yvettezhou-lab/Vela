...
export const LogbookView=({annualReflection,activeTrip,reflectionTrips}:Props)=>{
 ...
 return <main className="logbook-page">
  ...
  {selectedTrip&&<section className="logbook-section logbook-glance"><div className="logbook-section-heading"><div><span className="logbook-overline">THE JOURNEY AT A GLANCE</span><h2>One trip, clearly seen.</h2></div></div><div className="logbook-glance-grid"><article><span>TOTAL EXPENSE</span><strong>¥{num(total)}</strong></article><article><span>DAILY AVG</span><strong>¥{num(total/days(selectedTrip))}</strong></article><article><span>PER PERSON / DAY</span><strong>¥{num(total/Math.max(1,people)/days(selectedTrip))}</strong></article><article><span>ENTRIES</span><strong>{selectedTrip.ledger.filter(e=>countsInStats(selectedTrip,e)).length}</strong></article></div><div className="logbook-facts">{days(selectedTrip)} DAYS · {segmentCount} SEGMENTS · {people} PEOPLE</div></section>}
