        ...
        <div className="trip-allocation-list">
          {members.filter((member) => member.archived !== true).map((member) => (
            <label key={member.id} className="trip-allocation-row">
              ...
              <span>
                <input type="text" inputMode="decimal" value={Number.isFinite(Number(presetPercentages[member.id])) ? String(Number(presetPercentages[member.id])) : ""} onChange={(e) => { const raw = e.target.value.replace(/^0+(?=\d)/, ''); setPresetPercentages((current) => ({ ...current, [member.id]: raw === "" ? 0 : Number(raw) })); }} aria-label={member.name + " preset percentage"} />
                %
              </span>
