...
const CONFIGS: Config[] = [{ type: 'members', label: 'Persons', singular: 'Person' }, { type: 'categories', label: 'Categories', singular: 'Category' }, { type: 'accounts', label: 'Accounts', singular: 'Account' }];
...
const MasterDataSection: React.FC<{ tripId: string; config: Config; items: MasterDataItem[]; update: VelaStateUpdate; archive: (id: string) => void }> = ({ tripId, config, items, update, archive }) => {
  ...
  const save = () => { if (!name.trim()) return; const base = config.type === 'categories' ? { id: editingId ?? makeId('category'), name, type: categoryType } : config.type === 'accounts' ? { id: editingId ?? makeId('account'), name } : { id: editingId ?? makeId('person'), name }; const original = items.find((item) => item.id === editingId); update(tripId, config.type, editingId, { ...base, ...(original?.archived ? { archived: true } : {}) } as MasterDataItem); closeEditor(); };
  return <section className="master-data-card"><div className="master-data-card-head"><div><span className="master-data-kicker">MASTER DATA</span><h2>{config.label}</h2></div><button type="button" className="master-data-add" onClick={beginCreate}><Plus size={16} /> Add</button></div>
    ...
    {items.some(isArchived) && <button type="button" className="master-data-archived-toggle" onClick={() => setShowArchived((v) => !v)}>{showArchived ? 'Hide archived' : `Show archived (${items.filter(isArchived).length})`}</button>}</section>;
};
