...
const MasterDataSection: React.FC<{ tripId: string; config: Config; items: MasterDataItem[]; update: VelaStateUpdate; archive: (id: string) => void }> = ({ tripId, config, items, update, archive }) => {
  ...
  return <div className="master-data-group"><div className="master-data-group-head"><div><span className="master-data-kicker">{config.label.toUpperCase()}</span></div><button type="button" className="master-data-add" onClick={beginCreate}><Plus size={14} /> Add</button></div>
