...
const tripDays = (trip: Trip): number => Math.max(1, Math.floor((trip.endDate - trip.startDate) / DAY_MS) + 1);
...
export const calculateCrossTripIntelligence = (achieveTrips: Trip[]): CrossTripIntelligence => {
  ...
  const years = new Set(achieveTrips.map((trip) => new Date(trip.startDate).getFullYear()));
  const yearCount = Math.max(1, years.size);
  const byDestination = new Map<string, Trip[]>();
  achieveTrips.forEach((trip) => byDestination.set(trip.destination, [...(byDestination.get(trip.destination) ?? []), trip]));
