...
export function exportPlanExcel(plan: Plan) {
  ...
  const ledger = [['Date','Usage Start','Usage End','Usage Dates','Description','Category','Event','Amount','Currency','Payer','Account','Prepaid','Actual Dates','Final Amount','Final Currency'], ...plan.ledger.map(e => [e.date,e.usageStartDate,e.usageEndDate,(e.usageDates||[]).join(', '),e.description,e.category,eventName(plan,e.eventId),e.amount,e.currency,memberName(plan,e.payerId),accountName(plan,e.accountId),e.planned?'Yes':'No',(e.actualDates||[]).join(', '),e.finalAmount ?? '',e.finalCurrency ?? ''])];
