...
import { isPending, allocationFinal, dateRange, distributeAmount, type Plan } from './domain';
...
export function exportPlanExcel(plan: Plan) {
  ...
  const ledger = [['Date','Usage Start','Usage End','Description','Category','Event','Amount','Currency','Payer','Account','Prepaid','Actual Dates','Final Amount','Final Currency'], ...plan.ledger.map(e => [e.date,e.usageStartDate,e.usageEndDate,e.description,e.category,eventName(plan,e.eventId),e.amount,e.currency,memberName(plan,e.payerId),accountName(plan,e.accountId),e.planned?'Yes':'No',(e.actualDates||[]).join(', '),e.finalAmount ?? '',e.finalCurrency ?? ''])];
  ...
  plan.ledger.filter(e => !isPending(e,plan.settlementCurrency)).forEach(e => { const days=dateRange(e.usageStartDate,e.usageEndDate); const actual=e.actualDates?.length?e.actualDates:(e.planned?[]:days); const originalActual=actual.reduce((s,d)=>s+(distributeAmount(e.amount,days)[d]||0),0); if(!originalActual)return; const finalActual=e.finalAmount!*(originalActual/e.amount); balances.get(e.payerId)!.paid += finalActual; e.allocations.forEach(a => balances.get(a.memberId)!.borne += finalActual*(a.amount/e.amount)); });
...
export function exportPersonalBill(plan: Plan, memberId: string) {
  ...
  const rows = [['Date','Event','Description','Category','Trip Total','Currency','This Person Allocation','Allocation Currency','Final Allocation','Final Currency','Prepaid'], ...plan.ledger.flatMap(e => {
    const a = e.allocations.find(x => x.memberId === memberId); if (!a) return [];
    const days=dateRange(e.usageStartDate,e.usageEndDate); const actual=e.actualDates?.length?e.actualDates:(e.planned?[]:days); const parts=distributeAmount(e.amount,days);
    return actual.map(day=>[day,eventName(plan,e.eventId),e.description,e.category,e.amount,e.currency,+((parts[day]||0)*(a.amount/e.amount)).toFixed(2),e.currency,e.finalAmount==null?'':+((parts[day]||0)*(a.amount/e.amount)*(e.finalAmount/e.amount)).toFixed(2),e.finalCurrency??'',e.planned?'Yes':'No']);
  })];
