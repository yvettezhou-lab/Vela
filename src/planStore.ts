...
export function saveArchivedPlans(plans: ArchivedPlan[]) {
  localStorage.setItem(ARCHIVE_KEY, JSON.stringify(plans));
}
...
export function recordPlanHistory(plan: Plan, reason: PlanHistory['reason']) {
  ...
  history.unshift({ planId: plan.id, savedAt: new Date().toISOString(), reason });
...
export function archivePlan(plan: Plan): ArchivedPlan[] {
  ...
  archived.unshift({ plan: structuredClone(plan), archivedAt: new Date().toISOString() });
...
export function restoreArchivedPlan(id: string): Plan | undefined {
  ...
  saveActivePlanId(found.plan.id);
  recordPlanHistory(found.plan, 'restored');
  return found.plan;
}
