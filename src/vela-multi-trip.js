(() => {
  ...
  function makePlan() {
    const uid = () => (crypto.randomUUID ? crypto.randomUUID() : `trip-${Date.now()}-${Math.random().toString(16).slice(2)}`); const me = { id: uid(), name: 'Me', ratio: 100 };
    return { id: uid(), name: 'New Journey', startDate: '', endDate: '', destinations: [], settlementCurrency: 'CNY', status: 'Planning', members: [me], accounts: [{ id: uid(), name: 'Cash' }, { id: uid(), name: 'Bank Card' }, { id: uid(), name: 'Alipay' }, { id: uid(), name: 'WeChat Pay' }], events: [], ledger: [] };
  }
  ...
  function render() {
    const home = Array.from(document.querySelectorAll('.home-page'))[0]; if (!home) return; const { active, plans } = activeAndPlans(); if (!active?.id) return;
    const existing = home.querySelector(`#${ROOT_ID}`); if (existing) { const signature = JSON.stringify({active, count: plans.length}); if (existing.dataset.signature === signature) return; existing.remove(); }
    const others = plans.filter(p => p.id !== active.id && p.status !== 'Completed'); const hub = document.createElement('section'); hub.id = ROOT_ID; hub.dataset.signature = JSON.stringify({active, count: plans.length});
    hub.innerHTML = `
      <section class="vela-current-trip">
        <div class="vela-trip-section-head">
          ...
          <div class="vela-trip-actions"><span class="vela-trip-live">${esc(active.status || 'Planning')}</span><button type="button" class="vela-trip-plan">Plan</button></div>
        </div>
        <div class="vela-trip-meta"><span>${esc(destination(active))}</span><span>${esc(dates(active))}</span></div>
        <div class="vela-current-note"><strong>Quick Entry payments go here.</strong><small>${entries(active)} ledger ${entries(active) === 1 ? 'entry' : 'entries'} · ${esc(active.settlementCurrency || 'CNY')}</small></div>
      </section>
      <section class="vela-trip-list">
        ...
        ${others.length ? `<div class="vela-trip-cards">${others.map(p => `<button type="button" class="vela-trip-card" data-trip-id="${esc(p.id)}"><span class="vela-trip-card-main"><strong>${esc(p.name || 'New Journey')}</strong><small>${esc(destination(p))}</small><small>${esc(dates(p))}</small></span><span class="vela-trip-card-side"><i>${esc(p.status || 'Planning')}</i><b>${entries(p)}</b><small>entries</small></span></button>`).join('')}</div>` : `<div class="vela-trip-empty">No other trips in plan.<br><small>+ New Trip keeps another journey alongside this one.</small></div>`}
      </section>`;
    ...
    hub.querySelector('.vela-trip-plan')?.addEventListener('click', () => home.querySelector('.home-topbar .quiet')?.click());
