(() => {
  ...
  function seed() {
    ...
    if (p?.id) {
      ...
      writePlans(plans); localStorage.setItem(CURRENT_KEY, p.id); return;
    }
    if (plans.length) {
  ...
  function switchTrip(id) {
    const { plans } = activeAndPlans(); const target = plans.find(p => p.id === id); if (!target) return;
    localStorage.setItem(CURRENT_KEY, target.id); localStorage.setItem(PLAN_KEY, JSON.stringify(target)); location.reload();
  }
  function createTrip() {
    const { plans } = activeAndPlans(); const p = makePlan(); plans.unshift(p); writePlans(plans);
    localStorage.setItem(CURRENT_KEY, p.id); localStorage.setItem(PLAN_KEY, JSON.stringify(p)); location.reload();
  }

  function render() {
    const home = Array.from(document.querySelectorAll('.home-page'))[0]; if (!home) return;
    const { active, plans } = activeAndPlans(); if (!active?.id) return;
    const existing = home.querySelector(`#${ROOT_ID}`);
    if (existing) { const signature = JSON.stringify({active, count: plans.length}); if (existing.dataset.signature === signature) return; existing.remove(); }
    const others = plans.filter(p => p.id !== active.id && p.status !== 'Completed');
    const hub = document.createElement('section'); hub.id = ROOT_ID; hub.dataset.signature = JSON.stringify({active, count: plans.length});
    hub.innerHTML = `
      <section class="vela-current-trip">
        <div class="vela-trip-section-head">
          <div><span class="eyebrow">CURRENT TRIP</span><h3>Quick Entry → current trip</h3></div>
          <span class="vela-trip-live">${esc(active.status || 'Planning')}</span>
        </div>
        <div class="vela-trip-meta"><span>${esc(dates(active))}</span><span>${entries(active)} entries · ${esc(active.settlementCurrency || 'CNY')}</span></div>
      </section>
