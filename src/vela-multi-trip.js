(() => {
  ...
  function card(p, current, featured = false, number = '') {
    ...
    return `<button type="button" class="vela-trip-card${featured ? ' is-featured' : ''}${status === 'traveling' ? ' is-traveling' : ''}" data-trip-id="${esc(p.id)}"><span class="vela-trip-route" aria-hidden="true"><i></i><i></i><i></i></span><span class="vela-trip-card-main"><span class="vela-trip-topline"><span class="vela-trip-status">${label}</span><span class="vela-trip-number">TRIP ${esc(number)}</span></span><strong>${esc(p.name || 'New Journey')}</strong>${dest ? `<small class="vela-trip-destination">${esc(dest)}</small>` : ''}<small class="vela-trip-date">${esc(dates(p))}</small><span class="vela-trip-open">OPEN JOURNEY <b>↗</b></span></span></button>`;
  }
  ...
  function render() {
    ...
    hub.querySelector('.vela-trip-all')?.addEventListener('click', () => {
      ...
      stage.innerHTML = all.map((p, i) => card(p, current, i === 0, String(i+1).padStart(2,'0'))).join('');
      let secondaryBox = hub.querySelector('.vela-trip-secondary');
      if (!secondaryBox) { const label = document.createElement('div'); label.className = 'vela-trip-secondary-label'; label.textContent = 'ARCHIVE'; secondaryBox = document.createElement('div'); secondaryBox.className = 'vela-trip-secondary'; hub.append(label, secondaryBox); }
      const achieved = plans.filter(p => normalizeStatus(p) === 'achieve');
      secondaryBox.innerHTML = achieved.length ? achieved.map((p,i) => card(p,current,false,String(i+1).padStart(2,'0'))).join('') : '<div class="vela-trip-empty">No completed journeys yet.</div>';
      attachCards(hub);
    });
    attachCards(hub);
  }
