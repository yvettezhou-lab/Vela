...
export const useVelaStore = create<VelaState>()(persist((set, get) => ({
  ...
  addListTemplate: (tripId, template) => { const trips=get().trips; const trip=trips.find(t=>t.id===tripId); if(!trip) throw new Error(`Trip ${tripId} not found`); const existingLists=Array.isArray(trip.lists)?trip.lists:createDefaultTripLists(trip); const clean=template.name.trim(); if(existingLists.some(list=>list.name.trim().toLowerCase()===clean.toLowerCase())) throw new Error('A list with this name already exists'); const t=Date.now(); const list=createListFromTemplate(tripId, template, existingLists.length, existingLists); set({trips:trips.map(item=>item.id===tripId?{...item,lists:[...existingLists,list],updatedAt:t}:item)}); },
