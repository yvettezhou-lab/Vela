...
import { getSegmentPrimaryCurrency } from '../core/travelSegment';
...
export const useVelaStore = create<VelaState>()(persist((set, get) => ({
  ...
  addLedgerEntry: (tripId, rawEntry) => { const trips = get().trips; const tripIndex = trips.findIndex((t) => t.id === tripId); if (tripIndex === -1) throw new Error(`Store Error: Trip ${tripId} not found`); if (!isRecord(rawEntry)) throw new Error('Store Error: Entry must be an object'); const paymentDate = typeof rawEntry.paymentDate === 'number' ? rawEntry.paymentDate : undefined; const resolvedCurrency = paymentDate !== undefined ? getSegmentPrimaryCurrency(trips[tripIndex].segments, paymentDate) : undefined; const currencyResolvedEntry = resolvedCurrency ? { ...rawEntry, originalCurrency: resolvedCurrency } : rawEntry; const now = Date.now(); const baseTrip = withDefaultAccountsAndMember(withDefaultCategories(trips[tripIndex])); const strictTrip = DomainValidator.validateEntireTrip({ ...baseTrip, ledger: [...baseTrip.ledger, { ...currencyResolvedEntry, createdAt: now, updatedAt: now }] }, trips.filter((t) => t.id !== tripId)); set((state) => ({ trips: state.trips.map((t) => t.id === tripId ? strictTrip : t) })); },
