...
const normalizeTrips = (trips: Trip[]): Trip[] => refreshAutoTripTitles(trips.map((trip) => withDefaultAccountsAndMember(withDefaultCategories(trip))));
...
export const useVelaStore = create<VelaState>()(persist((set, get) => ({
  ...
  updateTripStatus: (tripId, newStatus) => { const trips = get().trips; const trip = trips.find((t) => t.id === tripId); if (!trip) throw new Error(`Store Error: Trip ${tripId} not found`); DomainValidator.validateTripStatus(trips, tripId, 'traveling', trip.status); if (newStatus !== 'planning' && newStatus !== 'traveling' && newStatus !== 'achieve') throw new Error(`Store Error: Invalid status ${newStatus}`); set({ trips: trips.map((t) => t.id === tripId ? { ...t, status: newStatus as TripStatus, updatedAt: Date.now() } : t) }); },
