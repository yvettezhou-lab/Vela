...
export const useVelaStore = create<VelaState>()(persist((set, get) => ({
  ...
  addListItem: (tripId,listId,title) => { const clean=title.trim(); if(!clean) throw new Error('Item is required'); const trips=get().trips; const trip=trips.find(t=>t.id===tripId); if(!trip) throw new Error('Trip not found'); const lists=Array.isArray(trip.lists)?trip.lists:createDefaultTripLists(trip, get().commonMembers); const target=lists.find(list=>list.id===listId); if(!target) throw new Error('List not found'); const key=listItemKey(clean); const duplicate=lists.some(list=>list.id!==listId&&list.items.some(item=>listItemKey(item.title)===key)); if(duplicate) throw new Error('This item already exists in another list'); const t=Date.now(); set({trips:trips.map(item=>item.id===tripId?{...item,lists:(Array.isArray(item.lists)?item.lists:createDefaultTripLists(item, get().commonMembers)).map(list=>list.id===listId?{...list,updatedAt:t,items:[...list.items,{id:crypto.randomUUID(),listId,title:clean,completed:false,sortOrder:list.items.length,createdAt:t,updatedAt:t}]}:list),updatedAt:t}:item)}); },
