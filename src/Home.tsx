...
const localSummary = (trip: Trip) => { const statsLedger = trip.ledger.filter(entry => entry.entryType !== 'cash_exchange' && trip.categories.find(category => category.id === entry.categoryId)?.excludeFromStats !== true); const local = statsLedger.reduce((sum, entry) => sum + (entry.isRefund ? -entry.originalAmount : entry.originalAmount), 0); const totals = calculateFinancialTotals(trip.ledger, trip.categories); return [dayCount(trip), local ? `${getTripPrimaryCurrency(trip)} ${Math.round(local).toLocaleString('en-US')}` : '', totals.financialTotal ? `¥${Math.round(totals.financialTotal).toLocaleString('en-US')}` : ''].filter(Boolean).join(' · '); };
